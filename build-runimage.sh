...
INSTALL_PKGS=(
    ...
    steam prismlauncher-git mcpelauncher-linux-git mcpelauncher-ui-git pipewire-alsa pipewire-pulse
    zenity chromium jre17-openjdk wget
)

rim-update # Update runimage

pac --noconfirm -S "${INSTALL_PKGS[@]}" # Install packages

rim-shrink --all # Shrink runimage
...
rim-build -s -b 1048576 -c 19 -z runimage-x64 # Rebuild runimage with zstd compression level 19 and 1M blocksize
}
